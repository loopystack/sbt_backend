...
async def initiate_withdrawal(
    ...
    if idempotency_key and (len(idempotency_key) < 10 or len(idempotency_key) > 255):
        raise HTTPException(
            status_code=400,
    ...
    async def _process_withdrawal():
        ...
        # Legacy idempotency check: If client_request_id provided, check if request already exists
    ...
    if idempotency_key:
        ...
        request_data = {
            ...
            "memo": withdrawal_data.memo,
            "client_request_id": withdrawal_data.client_request_id
        }

        status_code, response_data = await idempotency_service.process_idempotent_request(
            db=db,
            ...
            user_id=current_user.id,
            request_processor=_process_withdrawal
        )

        if status_code != 200:
            raise HTTPException(status_code=status_code, detail=response_data)

        return WithdrawalIntentResponse(**response_data)
    else:
        # No idempotency key, process normally
        status_code, response_data = await _process_withdrawal()
        return WithdrawalIntentResponse(**response_data)
